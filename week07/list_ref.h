...
   ListConstIterator<T> cbegin()  const
    {
        return ListConstIterator<T>(pHead);
    }
    ...
       ListConstIterator<T> crend()   const
    {
        return ListConstIterator<T>(NULL);
         
    }
...
   Node<T> *pTail;
        // pointer to the ending of the list
};
/*************************************************
 * LIST ITERATOR
...
  
    {
              if (p)
         return p->data;
              else          throw "ERROR: Trying to dereference a NULL pointer";
          
    }
...
         pTail->pNext = pNew;
                  else          pHead = pNew;
               // there is no tail so there is no head!
      
...
         pHead->pPrev = pNew;
                  else          pTail = pNew;
                   // there is no head so there is no tail!
...
T &List<T>::front() throw(const char *)
{
       if (pHead != NULL)
      return pHead->data;
       else       throw "ERROR: unable to access data from an empty list";
}
/*********************************************
 * LIST :: BACK
