...
 ******************************************************/
// //input is not yet identified, so this function will have to wait
//  void extractRecursive(BinaryNode <Pair <string, float> > * tree,string sPrefix
...
//             if (input(i).begin() == tree->data.getFirst())
//             {
...
//     assert(tree->pRight);
//     extractRecursive(tree->pLeft, sPrefix + "0", input, output);
    
//     // handle the right tree
//     assert(tree->pLeft);
//     extractRecursive(tree->pRight, sPrefix + "1", input, output);
// }
